...
const BlogCard = ({ index, slug, title, excerpt, image, date, read_time_min, isDragging }: BlogCardProps) => {
  ...
  return (
    <motion.article
      ...
      whileHover={{ y: -10 }}
      className="group relative z-20 bg-black rounded-2xl w-[320px] shrink-0 snap-start overflow-hidden select-none border border-[#915eff]/10 pointer-events-auto"
    >
      {/* Purple glow on hover - ignore pointer events */}
      <div className="absolute -inset-0.5 bg-gradient-to-r from-[#915eff]/0 via-[#915eff]/20 to-[#915eff]/0 rounded-2xl opacity-0 group-hover:opacity-100 blur-lg transition-opacity duration-500 pointer-events-none" />
      
      <div className="relative bg-black rounded-2xl overflow-hidden">
        <div className="aspect-video w-full overflow-hidden rounded-t-2xl">
          <img src={image} alt={title} className="w-full h-full object-cover" draggable="false" />
          {/* Purple overlay on hover - ignore pointer events */}
          <div className="absolute inset-0 bg-[#915eff]/0 group-hover:bg-[#915eff]/10 transition-colors duration-300 pointer-events-none" />
        </div>
        <div className="p-5">
          ...
          <Link
            to={`/blog/${slug}`}
            className="relative z-50 pointer-events-auto mt-4 inline-flex items-center gap-2 text-[14px] text-[#915eff]/60 hover:text-[#915eff] transition-colors cursor-pointer group/btn"
...
export const Feedbacks = () => {
  ...
  // Use framer-motion drag events to track dragging state
